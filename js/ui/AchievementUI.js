export class AchievementUI {
    ...
    showUnlockNotification(unlockData) {
        ...
        if (tier.reward.credits) {
            rewardTexts.push(`💰 ${tier.reward.credits} Credits`);
        }
        if (tier.reward.damageBonus) {
            rewardTexts.push(`⚔️ +${(tier.reward.damageBonus * 100).toFixed(0)}% Damage`);
        }
        if (tier.reward.healthBonus) {
            rewardTexts.push(`❤️ +${tier.reward.healthBonus} Max Health`);
        }
        if (tier.reward.shieldBonus) {
            rewardTexts.push(`🛡️ +${tier.reward.shieldBonus} Max Shield`);
        }
        if (tier.reward.creditMultiplier) {
            rewardTexts.push(`💎 +${(tier.reward.creditMultiplier * 100).toFixed(0)}% Credit Gain`);
        }
        if (tier.reward.title) {
            rewardTexts.push(`🏆 Title: "${tier.reward.title}"`);
        }
